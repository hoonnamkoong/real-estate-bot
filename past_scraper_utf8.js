    ...
    try {
        ...
        const listBtnHandle = await page.evaluateHandle(() => {
            return document.querySelector('.btn_list')
                || Array.from(document.querySelectorAll('button, a, div[role="button"]')).find(b => b.innerText.includes('紐⑸줉'))
        ...
        const data = list.map(item => {
            ...
            if (!areaM2 && item.spc2) {
                const m2Match = item.spc2.match(/(\d+(?:\.\d+)?)\s*(?:m짼|??/);
        ...
        const filteredData = data.filter(item => {
            ...
            if (item.tradTpNm !== '留ㅻℓ') return false;
